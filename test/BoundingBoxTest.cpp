...
protected:
	...
	box_2D = 0;
	box_3D = 0;
	initializeBox<1>(&box_1D);
	initializeBox<2>(&box_2D);
	initializeBox<3>(&box_3D);
    }
    ...
    template<int D> void initializeBox(BoundingBox<D> **box);
    template<int D> void deleteBox(BoundingBox<D> **box);

    template<int D> void testInitialization(const BoundingBox<D> *box);
...
template<int D>
void BoundingBoxTest::initializeBox(BoundingBox<D> **box) {
    ASSERT_TRUE(box != 0);
    ASSERT_TRUE(*box == 0);
    int n = -1;
    int l[D];
...
template<int D>
void BoundingBoxTest::testInitialization(const BoundingBox<D> *box) {
    ASSERT_TRUE(box != 0);
