 ...
 */
 ...
 bool initial_guess::sad::setup(OrbitalVector &Phi, double prec, double screen, const Nuclei &nucs, int zeta) {
     ...
     // Must build MRDFT before XCOperator
     auto mrdft_p = xc_factory.build();
     // Create a unique_ptr to pass to XCOperator
     std::unique_ptr<mrdft::MRDFT> mrdft_up = std::move(mrdft_p);
     XCOperator xc_op(mrdft_up, nullptr, false); // changed XC to xc_op for libxc debug
     
 
     RankZeroOperator V = V_nuc + J + xc_op;
     ...
     // Compute XC density
     Density &rho_xc = xc_op.getDensity(DensityType::Total);
 ...
 bool initial_guess::sad::setup(OrbitalVector &Phi, double prec, double screen, const Nuclei &nucs) {
     ...
     // Create a unique_ptr to pass to XCOperator
     std::unique_ptr<mrdft::MRDFT> mrdft_up = std::move(mrdft_p);
     XCOperator xc_op(mrdft_up, nullptr, false);
