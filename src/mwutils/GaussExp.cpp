...
GaussExp<D>::GaussExp(const GaussExp<D> &gexp) {
	...
	for (unsigned int i = 0; i < gexp.getNFuncs(); i++) {
...
GaussExp<D>::GaussExp(const GaussPoly<D> &gPoly) : screening(0.0), squareNorm(-1.0) {
	...
	for (int i = 0; i < nTerms; i++) {
		...
		if (coef != 0.0) {
			GaussFunc<D> *gFunc = new GaussFunc<D>(alpha, coef, pos, pow);
			this->addFunc(gFunc);
		}
	}
...
bool GaussExp<D>::checkSeedNode(MWNode<D> &node) {
	for (unsigned int i = 0; i < this->getNFuncs(); i++) {
...
template<int D>
void GaussExp<D>::setGaussFunc(int i, const GaussPoly<D> &g, double c) {
	if (i < 0 or i > (this->getNFuncs() - 1)) {
...
template<int D>
void GaussExp<D>::setGaussFunc(int i, const GaussFunc<D> &g, double c) {
	if (i < 0 or i > (this->getNFuncs() - 1)) {
		MSG_ERROR("Index out of bounds!");
		return;
	}
...
GaussExp<D> GaussExp<D>::add(GaussExp<D> & g) {
	int nsum = this->getNFuncs() + g.getNFuncs();
...
GaussExp<D> GaussExp<D>::add(Gaussian<D> &g) {
	int nsum = this->getNFuncs() + 1;
	GaussExp<D> sum = GaussExp<D> (nsum);
	for (int n = 0; n < this->getNFuncs(); n++) {
		sum.funcs[n] = this->getFunc(n).copy();
	}
	sum.funcs[this->getNFuncs()] = g.copy();
	return sum;
}
...
double GaussExp<D>::calcSquareNorm() {
	...
	for (int i = 0; i < this->getNFuncs(); i++) {
		for (int j = i + 1; j < this->getNFuncs(); j++) {
