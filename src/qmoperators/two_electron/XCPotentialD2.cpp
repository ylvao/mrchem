...
void XCPotentialD2::setupPerturbedDensity(double prec) {
    ...
    if (this->functional->isSpinSeparated()) {
        ...
        print_utils::qmfunction(2, "XC pert alpha density", dRho_a, time_a);

        Timer time_b;
        density::compute(prec, dRho_b, Phi, X, Y, DENSITY::Beta);
        print_utils::qmfunction(2, "XC pert beta density", dRho_b, time_b);
        ...
        print_utils::qmfunction(2, "XC pert total density", dRho_t, time_t);
    }
}
