...
public:
    XCOperator(std::shared_ptr<mrdft::XCFunctional> F, OrbitalVector *Phi = nullptr)
            : potential(new XCPotentialD1(F, Phi)) {
        RankZeroTensorOperator &XC = (*this);
        XC = *this->potential;
    }
    XCOperator(std::shared_ptr<mrdft::XCFunctional> F, OrbitalVector *Phi, OrbitalVector *X, OrbitalVector *Y)
