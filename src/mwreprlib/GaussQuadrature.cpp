...
GaussQuadrature::GaussQuadrature(int order, double a, double b, int intervals) :
	...
	if (order < 0 || order > MaxGaussOrder) {
		THROW_ERROR("Gauss quadrature order " << order <<
				" is larger than the maximum of " << MaxGaussOrder);
	}
	if (a >= b) {
		THROW_ERROR("Invalid Gauss interval, a > b.");
	}
	if (intervals < 1) {
		THROW_ERROR("Invalid number of intervals, intervals < 1");
	}
	...
	if (calcGaussPtsWgts() != 1) {
		THROW_ERROR("Setup of Gauss-Legendre weights failed.")
	}
	calcScaledPtsWgts();
}
...
void GaussQuadrature::setBounds(double a, double b) {
	...
	if (a >= b) {
		THROW_ERROR("Invalid bounds: a > b");
	}
	A = a;
	B = b;
	calcScaledPtsWgts();
...
void GaussQuadrature::setIntervals(int i) {
	...
	if (i < 1) {
		THROW_ERROR("Invalid number of integration intervals: " << i);
	}
	intervals = i;
	npts = order * intervals;
	roots = VectorXd::Zero(npts);
